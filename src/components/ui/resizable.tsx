...
const ResizableHandle = ({
  ...
  <ResizablePrimitive.PanelResizeHandle
    className={cn(
      ...
      "active:bg-purple-500/50 active:w-3",
  ...
  >
    {withHandle && (
      <div className="z-10 flex h-8 w-3 items-center justify-center rounded-sm bg-white/10 hover:bg-purple-500/40 transition-all duration-200 opacity-60 hover:opacity-100 active:opacity-100 active:bg-purple-500/60">
        <GripVertical className="h-3 w-2 text-white/60 hover:text-purple-200 transition-colors duration-200" />
      </div>
    )}
    
    {/* Resize indicator line */}
    <div className="absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-white/20 group-hover:bg-purple-500/60 group-active:bg-purple-500 transition-all duration-200 data-[panel-group-direction=vertical]:inset-x-0 data-[panel-group-direction=vertical]:top-1/2 data-[panel-group-direction=vertical]:h-px data-[panel-group-direction=vertical]:w-full data-[panel-group-direction=vertical]:-translate-y-1/2 data-[panel-group-direction=vertical]:translate-x-0"></div>
  </ResizablePrimitive.PanelResizeHandle>
)
