...
const NotesSidebar: React.FC<NotesSidebarProps> = ({
  ...
  const handleShareNote = async (service: 'onedrive' | 'googledrive' | 'device') => {
    ...
    if (success) {
      toast({
        title: "Note shared",
        description: `Note has been shared to ${service === 'onedrive' ? 'OneDrive' : service === 'googledrive' ? 'Google Drive' : 'your device'}`
      });
    } else {
      toast({
        title: "Error sharing",
  ...
  return <div className="bg-black/40 backdrop-blur-lg rounded-lg border border-white/10 shadow-lg flex flex-col text-white" style={{
    height: editorHeight ? `${editorHeight}px` : 'auto'
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-3">
          <Button variant="outline" size="sm" onClick={onSave} className="w-full mb-4 border-white/10 hover:bg-black/30 text-slate-300">
          ...
          <NotesList notes={notes} activeNoteId={activeNoteId} onLoadNote={handleLoadNote} onDeleteNote={handleDeleteNote} onOpenShare={handleOpenShare} formatNoteId={formatNoteId} />
        </div>
      ...
      <div className="p-3 border-t border-white/10">
        <Button variant="ghost" size="sm" className="w-full text-left justify-start text-white hover:bg-white/10" onClick={() => handleOpenShare(null)}>
      ...
      {/* Share Drawer */}
      <ShareNoteDrawer isOpen={isShareOpen} onOpenChange={setIsShareOpen} onShareNote={handleShareNote} />
    </div>;
};
